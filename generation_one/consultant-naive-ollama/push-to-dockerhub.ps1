# Push Consultant-Naive-Ollama to Docker Hub
...
Write-Host "   • Generates 100 expressions per batch (up from 5)" -ForegroundColor White
...
Write-Host "docker run --gpus all -p 11434:11434 -v `$(pwd)/credential.txt:/app/credential.txt $DockerHubUsername/consultant-naive-ollama:latest" -ForegroundColor White
